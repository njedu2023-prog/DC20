...
function boot(data) {
  ...
  const active = data.production_enabled === true;
  ...
  const labels={PENDING:'未到验证时间',MISSING:'真值缺失',PROMOTED:'晋级成功',NOT_PROMOTED:'未晋级',NO_FILL:'未买入（代理）',EXIT_BLOCKED:'退出受阻，待顺延',SETTLED:'已结算（代理）'};
  ...
  for(const r of ranked(rows,'promotion_rank')) {
    const v=verification.get(r.ts_code)||{};
    const slots=[];if(r.promotion_rank<=3)slots.push(`晋级 Top${r.promotion_rank}`);if(r.profit_rank<=2)slots.push(`盈利 Top${r.profit_rank}`);
    add('verification-table',[company(r,''),cell(slots.join(' / ')||'—'),cell(date(day.signal_date)),cell(date(day.exec_date)),cell(date(day.exit_date)),statusCell(v.t_status||'PENDING'),statusCell(v.t1_status||'PENDING'),number(v.net_return),cell(date(v.actual_exit_date))]);
  }
  ...
  for(const [key,label] of Object.entries(GROUPS)){
    const s=data.statistics?.groups?.[key]||{};
...
if (typeof module !== 'undefined') module.exports={pct,date,ranked,GROUPS};
