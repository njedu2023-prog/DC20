...
SCHEMA = "dc20_fixed_candidate_eligible_research_snapshot_20260922_v2"
REGISTRATION_ID = "dc20_fixed_ridge_999666_eligible_forward_20260922_v2"
CODE_ROOT = Path(__file__).absolute().parents[2]
REGISTRATION_PATH = Path(__file__).with_name("candidate_eligible_forward_registration.json")
...
from work.profit_1000_upgrade import candidate_eligible_pool as eligible


def _guard():
    require(Path(eligible.__file__).absolute() == CODE_ROOT / "work/profit_1000_upgrade/candidate_eligible_pool.py",
        "ELIGIBILITY_IMPORT_ORIGIN_CHANGED")
...
def registration():
    ...
    expected = {"schema_version": "dc20_fixed_candidate_natural_registration_v1",
        ...
        "eligibility_policy_id": eligible.POLICY_ID,
...
def freeze_natural_day(source_root, output_root, model_evaluation_path, *, signal_date,
    ...
    require(admitted["status"] in ("PROJECTED_UNVERIFIED_D_FEATURES", "EMPTY_P0_CANDIDATE_INPUT", "BLOCKED_DAILY_FEATURE_HISTORY")
    ...
    eligible.classify_projection(_plain(projection))  # Retain full-source validation; isolate only explicit stock history deficiencies.
    ...
    predictions = eligible.predict_eligible(model, _plain(projection), expected_model_sha256=MODEL_SHA)
    ...
    payload = {"schema_version": SCHEMA, "registration_id": REGISTRATION_ID,
        ...
        "promotion_slots": _slots([{**r, "candidate_rank": None, "candidate_score": None} for r in predictions["promotion_rows"]], "promotion_rank"),
        "promotion_top3": predictions["promotion_slots"],
        "entry_policy_id": scorer.ENTRY_POLICY_ID, "exit_policy_id": scorer.EXIT_POLICY_ID,
    ...
    return {"schema_version": "dc20_candidate_eligible_local_freeze_receipt_v2",
