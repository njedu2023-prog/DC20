"""Isolated v3 counterfactual labels; canonical price and capacity are separate.
...
from work.profit_1000_upgrade import auction_truth_v3 as auction_truth, minute_truth, policy_v3

SCHEMA = "dc20_profit_1000_research_candidates_v1"
LABEL_SCHEMA = "dc20_profit_1000_research_labels_v3"
...
def _source_policy(manifest: Mapping[str, Any]) -> dict:
    """This module only accepts v3; never silently falls through to old labels."""
    contract = policy_v3.validate_contract(manifest)
    if (contract["auction_source_policy_id"] != auction_truth.SOURCE_POLICY_ID
        ...
        raise ValueError("research v3 source adapter policy changed")
    return contract
...
def build_labels(repo_root: Path, manifest: Mapping[str, Any], *, as_of_date: str) -> dict:
    ...
    if source_policy is not None and any(row["shadow_max_price"] is not None for row in candidates):
        raise ValueError("v3 no-cap policy cannot silently consume a frozen entry cap")
    ...
    def canonical_price(day, code, daily_path, opening):
        try:
            ...
            row.update(entry_qualification_status=result["status"], price_qualified=result["price_qualified"],
                       capacity_amount=result["capacity_amount"], capacity_reason=result["capacity_reason"],
                       reported_auction_amount=result["reported_auction_amount"],
                       amount_identity_delta=result["amount_identity_delta"],
                       daily_open_cent_match=result["daily_open_cent_match"])
            return result
    ...
    for candidate in candidates:
        ...
        row.update(basis=policy_v3.BASIS, known_before_0925=False, price_reporting_precision_confirmed=False,
                   reported_price_preserved=True, price_qualified=False, capacity_amount=None,
                   reported_auction_amount=None, amount_identity_delta=None, daily_open_cent_match=None,
                   capacity_reason="ENTRY_NOT_OBSERVED", entry_qualification_status=None,
                   t_daily_volume=None, t_daily_ohlc_flat_at_cent=None, t_opening_limit_up_observed=None)
        output.append(row)
        ...
        try:
            ...
            # Use the existing half-up cent-tick price comparison, as used by
            # entry/exit truth; do not invent a wider floating-point tolerance.
            flat_at_cent = all(settlement._same_rounded_price(prices["close"], prices[key])
                               for key in ("open", "high", "low"))
            row.update(t_daily_volume=volume, t_daily_ohlc_flat_at_cent=flat_at_cent,
                       t_opening_limit_up_observed=settlement._same_rounded_price(prices["open"], up))
            if volume == 0 and not flat_at_cent:
                raise ValueError("zero T daily volume conflicts with non-flat OHLC")
            ...
            row.update(entry_price=price["price"], entry_price_source=price["entry_price_source"],
                       ...
                       capacity_proxy_verified=price["capacity_proxy_verified"])
            if str(price["status"]).startswith("PENDING_CANONICAL_"):
                ...
                continue
            if price["status"] not in {"CANONICAL_PRICE_OBSERVED", "DAILY_OPEN_PRICE_PROXY_CAPACITY_UNKNOWN", "OBSERVED_NO_AUCTION_TRADE"}:
                raise _ResearchSourceFailure("PENDING_INVALID_CANONICAL_AUCTION_SOURCE", "UnknownEntryQualification")
            status = None
            ...
            elif price["capacity_proxy_verified"] is True and price["amount"] is not None and price["amount"] * settlement.MAX_AUCTION_PARTICIPATION + 1e-9 < settlement.SHADOW_NOTIONAL_CNY:
            ...
            def minutes(day):
                ...
                if payload is None:
                    missing(day, "research_exit_1000_1m_0931", code)
                else:
    ...
    for day in sorted(manifest["expected_candidate_codes"]):
        ...
        complete = all(r["label_status"] == SETTLED or r["label_status"] in policy_v3.NO_FILL_STATUSES for r in rows)
        for row in rows:
            row["cohort_complete"] = complete
        cohorts[day] = {"expected_rows": len(rows), "terminal_rows": sum(r["label_status"] == SETTLED or r["label_status"] in policy_v3.NO_FILL_STATUSES for r in rows),
