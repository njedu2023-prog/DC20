"""Dormant formal-entry candidate. No production dispatch, writer, or CLI.
...
from work.profit_1000_upgrade import auction_truth, policy_v2

POLICY_ID = "dc20_primary_profit_shadow_canonical_auction_no_cap_v3"
POLICY_PATH = Path(__file__).with_name("FORMAL_ENTRY_V3_POLICY.json")
PREVIEW_SCHEMA = "dc20_formal_shadow_entry_v3_preparation_preview"
...
def policy_binding():
    ...
    policy = _json(raw)
    expected = {
        "schema_version": "dc20_formal_shadow_entry_policy_v3_preparation",
        ...
        "capacity_rule": "CANONICAL_AMOUNT_1_PERCENT_ELSE_PRICE_ONLY_CAPACITY_UNVERIFIED",
        ...
        "research_economic_reference": policy_v2.ENTRY_POLICY_ID,
    ...
    return {"path": "work/profit_1000_upgrade/FORMAL_ENTRY_V3_POLICY.json",
...
def activation_check(*, signal_date, frozen_at_utc, existing_signal_dates=(),
    ...
    blockers = ["PREPARATION_ONLY_NO_PRODUCTION_DISPATCH",
                ...
                "RESEARCH_V2_FULL_REPLAY_AND_FORWARD_ACCEPTANCE_REQUIRED"]
    if day in existing:
...
def _market_values(daily, limits):
    ...
    values = {key: _number(daily.get(key), positive=True) for key in ("open", "high", "low", "close", "pre_close")}
    volume = _number(daily.get("vol"))
    up = _number(limits.get("up_limit"), positive=True)
    down = _number(limits.get("down_limit"), positive=True)
    _require(down < up and down <= values["low"] <= min(values["open"], values["close"])
             <= max(values["open"], values["close"]) <= values["high"] <= up,
...
def preview_top2(root, *, signal_date, exec_date, exit_date, open_dates,
    ...
    for slot in (1, 2):
        ...
        rows.append({"shadow_slot": slot, "ts_code": candidate["ts_code"] if candidate else None,
                     ...
                     "entry_price_evidence": None, "slot_net_return": None, "net_return": None,
    ...
    for row in active:
        try:
            ...
            price = auction_truth.entry_price(source, t, row["ts_code"], str(values["open"]),
            ...
            elif price["amount"] is not None and _number(price["amount"]) * PARTICIPATION < NOTIONAL:
