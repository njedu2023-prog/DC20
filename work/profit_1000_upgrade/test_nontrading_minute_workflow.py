...
WORKFLOW = ROOT / ".github/workflows/research_nontrading_minute_gaps.yml"
...
def test_only_registered_plan_push_starts_a_single_nonretry_readonly_collection():
    ...
    assert value.get("on", value.get(True)) == {"push": {"branches": ["main"],
        "paths": ["work/profit_1000_upgrade/NONTRADING_MINUTE_GAP_COLLECTION.json"]}}
...
def test_process_budget_external_receipt_identity_and_unconditional_archive():
    ...
    assert "timeout --signal=TERM --kill-after=30s 5m" in collect
    ...
    assert uploads[0]["with"]["path"] == "${{ runner.temp }}/nontrading-minute-gaps/"
...
def test_isolated_supplement_plan_with_shared_serial_collection_budget():
    value = workflow()
    assert value["concurrency"]["group"] == "dc20-research-exit-minute-gaps"
    steps = value["jobs"]["minute-sources"]["steps"]
    code = [s.get("run", "") for s in steps]
    registered = "work/profit_1000_upgrade/NONTRADING_MINUTE_GAP_COLLECTION.json"
    registration = next(run for run in code if "c.read_plan" in run)
    assert registered in registration and 'assert 1 <= plan["planned_pair_count"] <= 6' in registration
    collect = next(run for run in code if "minute_gap_collect.py" in run)
    verify = next(run for run in code if "minute_gap_verify.py" in run)
    assert "--plan " + registered in collect
    assert "--registered-plan " + registered in verify
    assert "work/profit_1000_upgrade/MINUTE_GAP_COLLECTION.json" not in "\n".join(code)
