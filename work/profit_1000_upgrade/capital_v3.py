"""Explicit v3 source-aware replay of the frozen 45bp capital economics.

No label is converted to v2 or stripped of its auction source identity. Top1 and
Top2 are two independent CNY 1m diagnostics, not a combined CNY 1m portfolio.
The caller must independently verify label/source and frozen-ranking provenance.
This pure entry point never collects, trains, writes files or claims real fills.
"""
...
from work.profit_1000_upgrade.capital import (
    INITIAL_CASH, TARGET_NOTIONAL, COST_RATE, LOT_SIZE, SETTLED, EXIT_POLICY_ID,
    _date, _number, _money, _key, _at, _exit_time, _mark_source,
)

SCHEMA = "dc20_profit_1000_capital_candidate_overlay_v3"
...
def replay_capital_v3(ranked_rows, label_rows, *, open_dates, as_of_date, load_daily,
                      rank_field="candidate_rank", source_overlay_contract):
    ...
    """
    if type(source_overlay_contract) is not dict or _canonical(source_overlay_contract) != _canonical(overlay.CONTRACT):
        raise ValueError("EXACT_V3_SOURCE_OVERLAY_CONTRACT_REQUIRED")
    entry_policy_id = policy_v3.ENTRY_POLICY_ID
    ranked_rows, label_rows = list(ranked_rows), list(label_rows)
    ignored_ranks, ignored_labels = 0, 0
    ...
    frozen, day_ranks, ranked_seen = {}, {}, set()
    consumed_rankings = []
    for supplied in ranked_rows:
        ...
        if key[0] >= HOLDOUT_START:
            ignored_ranks += 1
            continue  # Future holdout ranks/features/outcomes are never inspected.
        consumed_rankings.append(supplied)
    ...
    labels, label_seen, consumed_labels = {}, set(), []
    for label in label_rows:
        ...
        if key[0] >= HOLDOUT_START:
            ignored_labels += 1
            continue  # Identity only: do not validate, copy, hash or read outcomes.
    ...
    for rank in (1, 2):
        ...
        for key, selected in sorted(frozen.items()):
            ...
            if label is not None:
                record.update({field: label.get(field) for field in (
                    "auction_source_policy_id", "minute_source_policy_id", "minute_time_semantics",
    ...
    return {"source_overlay_contract": deepcopy(source_overlay_contract),
            "auction_source_policy_ids_observed": sorted({r["auction_source_policy_id"] for r in consumed_labels}),
            ...
            "source_only_metadata_rewritten": False, "training_performed": False, "files_written": 0,
            "holdout_start_date": HOLDOUT_START, "holdout_role": "FUTURE_FORWARD_ONLY_NOT_EVALUATED",
            "holdout_ranked_rows_excluded_without_outcome_evaluation": ignored_ranks,
            "holdout_label_rows_excluded_without_outcome_evaluation": ignored_labels,
            "input_hash_scope": "DEVELOPMENT_ROWS_ONLY_HOLDOUT_IDENTITIES_COUNTED_NOT_HASHED",
            ...
            "ranking_input_sha256": hashlib.sha256(json.dumps(consumed_rankings, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()).hexdigest(),
            "label_input_sha256": hashlib.sha256(json.dumps(consumed_labels, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()).hexdigest(),
